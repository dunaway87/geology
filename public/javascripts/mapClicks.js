...
function addPopUp(bbox, epsg, x, y, width, height, latlng){
	if(neighborhoodClicked==false){
		...
		$.get(
				...
				"/geology/onClick?bbox="+bbox+"&epsg="+epsg+"&layers="+layerNames+"&x="+x+"&y="+y+"&height="+height+"&width="+width,function(result){
...
function appendToMyNeighborhood(layers,divToAppendTo,bbox, epsg, x, y, width, height, latlng){
	console.log("start");
	$.get("/geology/myNeighborhood?layers="+layers+"&bbox="+bbox+"&epsg="+epsg+"&x="+x+"&y="+y+"&height="+height+"&width="+width+"&lat="+latlng.lat+"&lon="+latlng.lng,function(result){
		console.log(result);
